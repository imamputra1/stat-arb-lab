...
if TYPE_CHECKING:
    from core.shared import OHLCV, FetchJob, Result
...
class CCXTAsyncAdaptor:
    ...
    def __init__(self, exchange_id: str='binance') -> None:
        ...
        try:
            self.exchange = getattr(ccxt, exchange_id)({'enableRateLimit': True, 'timeout': 30000, 'options': {'defaultType': 'spot'}})
    ...
    async def _fetch_page(self, job: 'FetchJob', cursor_ms: int) -> 'Result[Tuple[List[OHLCV], int], str]':
        from core.shared import Ok, Err, OHLCV
        max_retries = 3
        last_exception = None
        for attempt in range(max_retries):
            try:
                batch = await self.exchange.fetch_ohlcv(symbol=job.symbol, timeframe=job.timeframe, since=cursor_ms, limit=1000)
                if not batch:
                    return Ok(([], cursor_ms))
                parsed_candles = []
                ...
                for row in batch:
                    try:
                        candle = OHLCV(timestamp=int(row[0]), open=float(row[1]), high=float(row[2]), low=float(row[3]), close=float(row[4]), volume=float(row[5]))
