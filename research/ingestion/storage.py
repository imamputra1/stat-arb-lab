...
from core.shared import Result, Ok, Err, OHLCV, FetchJob
...
class ParquetStorageAdaptor:
    ...
    async def _process_single_month(self, month_df: pd.DataFrame, month_key: str, job: FetchJob) -> Result[str, str]:
        try:
            ...
            try:
                await asyncio.to_thread(pq.write_table, final_table, save_path, compression='zstd', compression_level=3)
            except Exception as e:
    ...
    def _get_monthly_save_path(self, month_key: str, job: FetchJob) -> Path:
        ...
        try:
            ...
            save_dir = self.base_path / f'symbol={safe_symbol}' / f'interval={job.timeframe}' / f'year={year}' / f'month={month.zfill(2)}'
    ...
    async def get_file_size(self, symbol: str, timeframe: str, month_key: str) -> Result[Optional[int], str]:
        try:
            safe_symbol = symbol.replace('/', '-')
            year, month = month_key.split('-')
            file_path = self.base_path / f'symbol={safe_symbol}' / f'interval={timeframe}' / f'year={year}' / f'month={month.zfill(2)}' / 'data.parquet'
            if file_path.exists():
