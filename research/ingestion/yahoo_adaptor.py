...
if TYPE_CHECKING:
    from core.shared import Result, OHLCV, FetchJob
...
class YahooFinanceAdaptor:
    ...
    def __init__(self) -> None:
        self._timeframe_map = {'1m': '1m', '2m': '2m', '5m': '5m', '15m': '15m', '30m': '30m', '60m': '60m', '90m': '90m', '1h': '60m', '1d': '1d', '5d': '5d', '1wk': '1wk', '1mo': '1mo', '3mo': '3mo'}
    ...
    async def fetch(self, job: 'FetchJob') -> 'Result[List[OHLCV], str]':
        try:
            if not job.symbol:
                return self._err('Job missing symbol')
    ...
    def _prepare_yahoo_params(self, job: 'FetchJob') -> Dict[str, Any]:
        ...
        params = {'start': start_str, 'end': end_str, 'interval': self._timeframe_map[job.timeframe], 'progress': False, 'auto_adjust': True, 'group_by': 'column'}
        return params
    ...
    def _parse_to_ohlcv(self, df: pd.DataFrame, symbol: str) -> List['OHLCV']:
        ...
        for row in df.itertuples():
            try:
                ...
                candle = OHLCV(timestamp=ts_ms, open=float(o), high=float(h), low=float(l), close=float(c), volume=float(v))
