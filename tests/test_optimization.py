...
from research.strategy.optimization import HyperParallelEngine, QuantumScoreKeeper, ScoringStrategy, QuantumParameterSpace
...
def test_scorekeeper_penalties():
    ...
    df_dead = pl.DataFrame({'cumulative_returns': np.zeros(100), 'position': np.zeros(100)})
...
def test_shotgun_initialization():
    ...
    try:
        engine = HyperParallelEngine(n_jobs=2)
